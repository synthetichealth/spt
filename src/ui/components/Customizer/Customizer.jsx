  ...
  return (
    <div className={classes.customizerContainer}>
      <Paper elevation={3} className={classes.dashboardCard}>
        ...
        {mode == 0 && (
          <Fragment>
            Welcome to the Synthea™ customizer. This page is meant to help users install, configure,
            and run Synthea for common use cases.
            <Stack
            ...
            >
              ...
              >
                ...
                <br />
                Guided Mode will walk you through common Synthea configuration options with a series
                of questions. This mode is recommended for first-time users and beginners.
              </Button>

              <Button
                variant="contained"
                onClick={() => setMode(ADVANCED_MODE)}
                style={{
                  display: 'block',
                  textTransform: 'none',
                  minWidth: 'min-content',
              ...
              >
                ...
                <br />
                Advanced Mode shows all customization options right away. Recommended for users who
                know exactly what customizations they want.
              </Button>
            </Stack>
          </Fragment>
        )}
  ...
  return (
    <React.Fragment>
      ...
      <ConfigFileBuilder
        config={config}
        setConfig={setConfig}
        configAsArgs={configAsArgs}
        setConfigAsArgs={setConfigAsArgs}
      />
      ...
      <DockerfileBuilder
        ...
        config={config}
        configAsArgs={configAsArgs}
        keepModuleString={keepModuleString}
      />
    </React.Fragment>
  );
};
...
const GuidedMode = () => {
  ...
  return (
    <React.Fragment>
      <ExportFormatsQuestion exportFormats={exportFormats} setExportFormats={setExportFormats} />{' '}
      <br /> <br />
      {exportFormats.length > 0 && (
        <DataReqsQuestion dataReqs={dataReqs} setDataReqs={setDataReqs} />
      )}
      {dataReqs.length > 0 && (
        <ArgBuilder args={args} setArgs={setArgs} onlyGroup="Basic" onlyRenderFields={true} />
      )}
      {dataReqs.includes('keep') && (
        <KeepModuleBuilder setKeepModuleString={setKeepModuleString} showDownload={false} />
      )}
      {dataReqs.includes('geographic') && (
        <ArgBuilder args={args} setArgs={setArgs} onlyGroup="Geographic" onlyRenderFields={true} />
      )}
      {dataReqs.includes('demographic') && (
        <ArgBuilder args={args} setArgs={setArgs} onlyGroup="Demographic" onlyRenderFields={true} />
      )}
      {dataReqs.includes('reproducibility') && (
        <ArgBuilder
          args={args}
          setArgs={setArgs}
          onlyGroup="Reproducibility"
          onlyRenderFields={true}
        />
      )}
      {dataReqs.length > 0 && (
        <TargetedConfigWrapper
          config={config}
          setConfig={setConfig}
          configAsArgs={configAsArgs}
          setConfigAsArgs={setConfigAsArgs}
...
const ExportFormatsQuestion = (props) => {
  ...
  return (
    <React.Fragment>
      <h3>Which data formats do you need?</h3>
      <br />
      <ToggleButtonGroup
        size="large"
        color="primary"
      ...
      >
        {formatsToShow.map((f) => (
          <ToggleButton key={f.key} value={f.key} aria-label={f.display}>
            {f.display}
          </ToggleButton>
        ))}
      </ToggleButtonGroup>
...
const DataReqsQuestion = (props) => {
  ...
  return (
    <React.Fragment>
      <h3>Which of the following data requirements apply to you?</h3>
      <br />
      <ToggleButtonGroup
        size="large"
        color="primary"
      ...
      >
        <ToggleButton value="keep">
          I need patients that meet certain clinical criteria. (Conditions, Observations,
          Procedures, etc)
        </ToggleButton>
        <ToggleButton value="geographic">I need a certain geographic location.</ToggleButton>
        ...
        <ToggleButton value="none">None of these - I just need some records.</ToggleButton>
      </ToggleButtonGroup>
      <br /> <br />
    </React.Fragment>
  );
...
const GuidedModeHowToRun = (props) => {
  ...
  return (
    <React.Fragment>
      <h3>How do you want to run Synthea?</h3>
      <br />
      <ToggleButtonGroup
        size="large"
        color="primary"
      ...
      >
        <ToggleButton
          value="docker"
          style={{ display: 'block', textTransform: 'none', maxWidth: '350px' }}
        >
          <b>Docker</b>
          <br />
          The Docker setup packages everything together and is a good choice for users whose desired
          changes are fully reflected above. The Docker setup of Synthea is generally easy to get
          running locally, but customization beyond the basics is difficult.
          <br />
          <br />
        ...
        >
          <b>Basic Setup</b>
          <br />
          The Basic setup is recommended for users who want to get started quickly and do not
          anticipate making significant changes or customizations to Synthea.
          <br />
          <br />
        ...
        >
          <b>Developer Setup</b>
          <br />
          The Developer setup is recommended for users who want the ability to fully modify and
          customize all aspects of Synthea.
          <br />
          <br />
...
const Instructions = (props) => {
  const { mode, args, keepModuleString, exportFormats, config } = props;
  ...
  if (exportFormats.includes('bulk_fhir')) {
    ...
    if (
      !configForExport['exporter.fhir.export'] &&
