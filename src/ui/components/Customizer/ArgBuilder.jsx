...
const COMMAND_LINE_ARGS = {
  ...
  age: {
    ...
    flag: '-a',
    group: 'Demographic',
    rangeToString: ageArrayToString,
  },

  seed: { display: 'Seed', type: 'number', flag: '-s', group: 'Reproducibility' },
  clinicianSeed: {
    display: 'Clinician Seed',
    type: 'number',
    flag: '-cs',
    group: 'Reproducibility',
  },
  referenceDate: {
    display: 'Reference Date (YYYYMMDD)',
    type: 'text',
    flag: '-r',
    group: 'Reproducibility',
  },
...
const GROUP_DESCRIPTIONS = {
  ...
  Geographic:
    'Select the geographic region to generate patients for. Synthea can only run for one state or one city' +
    ' at a time, defaulting to the state of Massachusetts',
  Demographic:
    'By default Synthea will generate patients matching the demographic characteristics ' +
    ' of the selected geographic region based on the US Census. These settings allow you to define alternate demographics.',
  Reproducibility: (
    <Fragment>
      Synthea allows users to exactly recreate a dataset when the same input parameters are provided
      to the same version of Synthea. At minimum, a <code>seed</code>, <code>clinician seed</code>,
      and <code>reference date</code> must be provided. The <code>seed</code> and{' '}
      <code>clinician seed</code> are used to pick random numbers used in the simulation, so the
      seeds chosen do not translate directly into specific results; they just need to be kept the
      same between runs. Some examples for using this are to grow/age a population over time, or to
      change something else in the simulation and see how that change impacts the results over time.
      <br />
      See{' '}
      <a
        href="https://github.com/synthetichealth/synthea/wiki/Recreating-a-Dataset"
        target="_blank"
        rel="noreferrer"
      >
        https://github.com/synthetichealth/synthea/wiki/Recreating-a-Dataset
      </a>{' '}
...
const ArgBuilder = (props) => {
  ...
  return (
    <div className={classes.collection}>
      ...
      {Object.entries(fields).map(([groupName, argFieldArray]) => {
        return (
          <div key={groupName}>
      ...
      <br />
      {!onlyRenderFields && (
        <BashCodeBlock code={renderArgs('java -jar synthea-with-dependencies.jar', args)} />
      )}
    </div>
  );
};
