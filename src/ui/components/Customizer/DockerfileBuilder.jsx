...
import { Paper, Button, Accordion, AccordionSummary, AccordionDetails } from '@mui/material';
...
const buildFileInDockerRun = (fileContent, targetFileName) => {
  ...
  const fileLines = fileContent
    .replaceAll("'", "'\"'\"'") // yes this is real. https://stackoverflow.com/a/1250279
    .split('\n');
...
const DockerfileBuilder = (props) => {
  ...
  if (keepModuleString) {
    dockerfile = dockerfile.replace(
      '%%KEEP_MODULE%%',
      buildFileInDockerRun(keepModuleString, 'keep.json'),
    );
  ...
  if (configAsArgs || Object.keys(config).length === 0) {
    ...
    if (configFileString) {
      dockerfile = dockerfile.replace(
        '%%CONFIG_FILE%%',
        buildFileInDockerRun(configFileString, 'custom.properties'),
      );
  ...
  return (
    <div className={classes.collection}>
      ...
      <Button
        variant="contained"
        onClick={() => saveFile(dockerfile, 'Dockerfile')}
      ...
      <p>
        Open powershell or terminal, navigate to where you downloaded the above Dockerfile, and run
        the following commands:
      </p>
      <p>
        1. Create your customized Synthea Docker container, we&apos;ll call it{' '}
        <code>syntheadocker</code>:
      </p>
      <BashCodeBlock code="docker build --tag syntheadocker - < Dockerfile" />
      <br />
      <p>
        2. Create an output folder for synthetic data called <code>docker_output</code>:
      </p>
      ...
      <br />
      Once the process completes, your <code>docker_output</code> folder will contain a subfolder
