...
export const lastCodeableConcept = list => (Array.isArray(list) && list.length > 0 ? list[list.length - 1] : undefined);

export const dateValue = (value, fieldName = 'date') => value || missingField(fieldName);

export const periodStart = (period, fieldName = 'period.start') => period?.start || missingField(fieldName);
